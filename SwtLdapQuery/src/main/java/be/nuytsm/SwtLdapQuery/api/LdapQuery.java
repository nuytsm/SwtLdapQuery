...
public class LdapQuery {
	...
	final static String ldapAdServer[] = {
			"ldap://CDCDC002.idcn.mil.intra:389",
			"ldap://CDCDC003.idcn.mil.intra:389" };
	...
	public SearchResult queryByAccountName(String accountName)
			throws NamingException {
		if (null == accountName || accountName.equalsIgnoreCase("")) {
			return null;
		}
		...
		SearchResult srLdapUser = this.findAccountByAccountName(ctx,
				ldapSearchBase, accountName);
		return srLdapUser;
		// try {
		// NamingEnumeration<? extends Attribute> atts =
		// srLdapUser.getAttributes().getAll();
		// PrintWriter writer = new
		// PrintWriter("c:/temp/ldapQuery "+ldapAccountToLookup +" "+
		// ldapAdServer.substring(10,15)+".txt", "UTF-8");
		//
		// while (atts.hasMore()) {
		...
		// String primaryGroupName = ldap.findGroupBySID(ctx, ldapSearchBase,
		// primaryGroupSID);
		// writer.println(primaryGroupName);
		// writer.close();
		// } catch (NullPointerException e) {
		// System.out.println("account " + ldapAccountToLookup +
		// " not found in " + ldapAdServer );
		// }
	}
	...
	public String processSearchResultToString(SearchResult sr)
			throws NamingException {
		if (null == sr){
		...
		NamingEnumeration<? extends Attribute> atts = sr.getAttributes()
				.getAll();
		...
		// 3) get the users Primary Group
		String primaryGroupName = this.findGroupBySID(getLdapContext(),
				ldapSearchBase, primaryGroupSID);
